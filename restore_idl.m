    ...
    for i=1:nargin
        if ischar(varargin{i})
            ...
            if fp > 0
                ...
                if strcmpi(mystring,'verbose')
                    verbose=1;
                end
                if strcmpi(mystring,'restore_verbose')
                    frestore_verbose=1;
                end
                if strcmpi(mystring,'lowercase')
                    dolower=1;
                end
                if strcmpi(mystring,'debug')
                    frestore_verbose=1;
                    verbose=1;
                end
...
readstart=0;
for nel=1:nelements
    if frestore_verbose && nelements > 1
        disp(['reading structure ' num2str(nel) ' of ' num2str(nelements)]);
    end
...
if nelements==1
    thevar=thevar{1};  % no need for cell if only one struct
end
end
...
readstart=1;
if nargin > 4
    readstart=varargin{2};
end
strucdesc=[];
if nargin > 5
    strucdesc=varargin{3};
end
